...
function doRectanglesOverlap(rect1, rect2) {
  const isOverBy = (length, coord1, coord2) => (coord1 + length) > coord2;
  const {
    top: top1,
    left: left1,
    width: width1,
  ...
  const isOverByWidth = isOverBy(width1, left1, left2);
  const isOverByHeight = isOverBy(height1, top1, top2);
  return isOverByHeight && isOverByWidth;
}
...
function isInsideCircle(circle, point) {
  ...
  const getLeg = (pointCoord, centerCoord) => {
    if ((pointCoord < 0 && centerCoord < 0)
      || (pointCoord >= 0 && centerCoord >= 0)) {
      return Math.abs(pointCoord) - Math.abs(centerCoord);
    }
    return Math.abs(pointCoord) + Math.abs(centerCoord);
  };

  const legX = getLeg(point.x, circle.center.x);
  const legY = getLeg(point.y, circle.center.y);
...
function findFirstSingleChar(str) {
  const arr = str.split('');
  const uniqChars = arr.filter(
    (char, index, array) => array.indexOf(char) === array.lastIndexOf(char),
  );
  return uniqChars && uniqChars.length > 0 ? uniqChars[0] : null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${leftBracket}${firstNumber}, ${secondNumber}${rightBracket}`;
}
...
function reverseInteger(num) {
  const resultStr = num.toString().split('').reverse().join('');
...
function isCreditCardNumber(ccn) {
  const numbersArr = [...String(ccn)].reverse();

  const sum = numbersArr.reduce((acc, current, index) => {
...
function getDigitalRoot(num) {
  let resultStr = [...(num.toString())].reduce(
    (acc, number) => acc + parseInt(number, 10), 0,
  ).toString();

  while (resultStr.length > 1) {
    resultStr = (parseInt(resultStr[0], 10) + getDigitalRoot(resultStr.slice(1))).toString();
  }
  return parseInt(resultStr[0], 10);
}
...
function evaluateTicTacToePosition(position) {
  const checkWinner = (char, board) => {
    ...
    return winMap.some((comb) => {
      const [ind0, ind1, ind2] = comb.split('');
      return board[ind0] === char
        && board[ind1] === char
        && board[ind2] === char;
    });
  };

  const board = position.map((row) => {
    if (row.length < 3) {
