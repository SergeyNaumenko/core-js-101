...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((resolve, reject) => {
    ...
    const errorMsg = 'Wrong parameter is passed! Ask her again.';
...
function chainPromises(array, action) {
  const chain = array.reduce((promiseChain, currentTask) => promiseChain.then(
    (chainResult) => currentTask.then((currentResult) => action(chainResult, currentResult)),
  ));

  return chain.then((response) => response).catch(() => 'Error');
}
